...
=head1 SYNOPSIS
  ...
  my $site=uSAC::HTTP::Site->new(id=>STRING, prefix=>STRING, delegate=>OBJECT/PACKAGE);
...
A delegate allows external code to modify add routes and middleware and reolve implicit route paths
