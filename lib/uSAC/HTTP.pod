...
uSAC::HTTP  - Micro Swiss Army Chainsaw HTTP


=head1 DESCRIPTION

uSAC::HTTP is a webserver implementing HTTP/1.1. It utilises persistant connections and a fast dispatching system to achieve solid performance in vanilla Perl with AnyEvent.
...
A uSAC program is made of only a few statments which are normal perl code, however have similarities with nginx config files

=item *Concise mapping

A single entry can specify a HTTP method , URL, fields to capture, middleware and target endpoint. Many of these are optional too.
